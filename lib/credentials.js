...
function _checkCoin(coin) {
  if (!_.includes(['btc', 'bch', 'cps'], coin)) throw new Error('Invalid coin');
};
...
Credentials.create = function(coin, network) {
  ...
  x.coin = coin;
  x.network = network;
  x.xPrivKey = (new Bitcore_[coin].HDPrivateKey(network)).toString();
...
Credentials._getNetworkFromExtendedKey = function(xKey, coin) {
  ...
  if (coin == 'cps') return xKey.charAt(0) == 'x' ? 'testnet' : 'livenet';
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    var xPrivKey = new Bitcore_[this.coin].HDPrivateKey.fromString(this.xPrivKey);
...
Credentials.prototype.getBaseAddressDerivationPath = function() {
  ...
  switch (this.derivationStrategy) {
   ...
   if (this.coin != 'cps')
    ...
    var coin = (this.network == 'livenet' ? "468" : "1");
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var xPrivKey = new Bitcore_[this.coin || 'btc'].HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
