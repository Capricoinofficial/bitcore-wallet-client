...
var Bitcore_ = {
  ...
  cps: require('capricoinplus-bitcore-lib'),
};
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Bitcore_[txp.coin || 'btc'].HDPrivateKey(derivedXPrivKey);
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['btc', 'bch', 'part'], coin));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'btc';
  if (!_.includes(['btc', 'bch', 'part'], coin)) return cb(new Error('Invalid coin'));
 ...
 * @param {String} copayerName
 * @param {Object} opts
 * @param {string} opts.coin[='btc'] - The expected coin for this wallet (btc, bch, part).
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['btc', 'bch', 'part'], opts.coin)) return cb(new Error('Invalid coin'));
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new Bitcore_[this.credentials.coin].PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new Bitcore_[this.credentials.coin].HDPrivateKey(this.credentials.xPrivKey)
    .deriveChild(this.credentials.getBaseAddressDerivationPath());
  var sig = Utils.signRequestPubKey(requestPubKey, xPriv, this.credentials.coin);
 ...
 */
API.prototype.updateColdStakingSetup = function(opts, cb) {
  var self = this;
  self._doPutRequest('/v1/coldstakingsetup/', opts, cb);
};
 ...
 */
API.prototype.getColdStakingSetup = function(cb) {
  var self = this;
  self._doGetRequest('/v1/coldstakingsetup/', cb);
};
 ...
 */
API.prototype.getColdStakingAddresses = function(cb) {
  var self = this;
  self._doGetRequest('/v1/coldstakingaddresses/', cb);
};
